...
# iris데이터, Sepal.Length, Sepal.Width 활용해서 종별로 산점도를 기리기
#제목과 x축을 변경
...
p2 <- p + 
  ...
  annotate(geom = "text", x = 6.5, y = 4, label = "versicolor", size = 5) + 
...
g + 
  geom_text(aes(label = rownames(mtcars)), hjust = 0, nudge_x = 0.1)
...
seoulair <- seoulair %>%
  rename(data="날짜",
         ...
         pm10="미세먼지",
         pm2.5="초미세먼지") %>%
  select(data, district, pm10, pm2.5)

table(seoulair$data)
table(seoulair$district)
seoulair <- seoulair %>% filter(data!="전체" & district!="평균")
...
seoulair %>%
  ...
  select(data, district, pm10)# 3개 변수만 출력

# 구별 미세먼지 평균 비교
seoulair %>%
  filter(!is.na(pm10)) %>%    # 결측치 없는 pm10 자료만 추출
  group_by(district) %>%      # 구를 기준으로 데이터 분류
  summarise(m=mean(pm10)) %>% # 구별로 pm10의 평균구하기
  arrange(m) %>%              # m을 기준으로 오른차순
  head(5)                     # 상위 5개행 출력
